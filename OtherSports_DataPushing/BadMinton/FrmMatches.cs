    ...
    public partial class FrmMatches : Form
    {
        ...
        void bindData()
        {
            try
            {
                ...
                cbTeamA.DataSource = objMB.ExeQueryStrRetDTBL("Select TeamID,TeamName from Team_Master Where SportID=" + mycommon.SportID, 1);
                cbTeamA.DisplayMember = "TeamName";
                cbTeamA.ValueMember = "TeamID";
                cbTeamA.SelectedIndex = -1;

                cbTeamB.DataSource = objMB.ExeQueryStrRetDTBL("Select TeamID,TeamName from Team_Master Where SportID=" + mycommon.SportID, 1);
        ...
        private void btnsubmit_Click(object sender, EventArgs e)
        {
            if (mycommon.SportID == 1)
            {
                try
                {
                    ...
                    if (objMB.ExeQueryRetBooDL("Insert Into BM_Matches (MatchId,CompID,MatchType,TeamA,TeamB,VenueId,Date,Type)values(" + ID + "," + cbCompetition.SelectedValue + "," + cbMatchtype.SelectedValue + "," + cbTeamA.SelectedValue + "," + cbTeamB.SelectedValue + "," + cbVenue.SelectedValue + ",'" + dtpMatchDate.Value + "','" + type + "')", 1))
                    {
