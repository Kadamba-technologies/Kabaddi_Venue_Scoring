...
{
    public partial class FrmMatches_BB : Form
    {
        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            if (btnEdit.Text == "Edit")
            {
                ...
                pageload = false;
                bindData();
                ID = objMB.ExeQueryStrIntBL("Select Isnull(Max(MatchId),0)+1 from BB_Matches", 1);
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (btnSubmit.Text == "Submit")
                {
                    ...
                    Success = objMB.ExeQueryRetBooDL("Insert Into BB_Matches (MatchId,CompetitionId,VenueID,TeamA,TeamB,Date,MatchTypeID,ClubAColor,ClubBColor,StageType)values(" + ID + "," + cbCompetition.SelectedValue + "," + cbVenue.SelectedValue + "," + cbTeamA.SelectedValue + "," + cbTeamB.SelectedValue + ",'" + dtpMatchDate.Value + "'," + cbMatchType.SelectedValue + ",'" + mycommon.TeamAColor + "','" + mycommon.TeamBColor + "','"+cbstage.SelectedText+"')", 1);
                ...
                else if (btnSubmit.Text == "Update")
                {
                    ...
                    Success = objMB.ExeQueryRetBooDL("Update BB_Matches set CompetitionId=" + cbCompetition.SelectedValue + ",VenueID=" + cbVenue.SelectedValue + ",TeamA=" + cbTeamA.SelectedValue + ",TeamB=" + cbTeamB.SelectedValue + ",Date='" + dtpMatchDate.Value + "',MatchTypeID=" + cbMatchType.SelectedValue + ",ClubAColor='" + mycommon.TeamAColor + "',ClubBColor='" + mycommon.TeamBColor + "',StageType='"+cbstage.SelectedText+"' where MatchId=" + lblMatchID.Text.Trim() + "", 1);
        ...
        void bindData()
        {
            ...
            if (mycommon.SportID == 14)
            {
                ...
                cbstage.DataSource = objMB.ExeQueryStrRetDTBL("Select distinct StageType Stage from BB_Matches", 1);
        ...
        void bindMatches()
        {
            cbName.DataSource = objMB.ExeQueryStrRetDTBL("Select MatchID,CAST(MatchID As Varchar)+' - '+dbo.fn_GetMatchName_BB(MatchId)Name from BB_Matches Order by date desc", 1);
        ...
        private void cbName_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (cbName.SelectedIndex != -1)
                {
                    ...
                    DT = new DataTable();
                    DT = objMB.ExeQueryStrRetDTBL("Select * from BB_Matches where MatchID=" + cbName.SelectedValue + "", 1);
                    if (DT.Rows.Count > 0)
                    {
        ...
        {
            GlobalAssingvalues();
            FrmLineups_BB frmLine = new FrmLineups_BB();
            frmLine.Show();
        }
        ...
        private void FrmMatches_BB_Load(object sender, EventArgs e)
        {
            bindMatches();
            bindData();
            ID = objMB.ExeQueryStrIntBL("Select Isnull(Max(MatchId),0)+1 from BB_Matches", 1);
