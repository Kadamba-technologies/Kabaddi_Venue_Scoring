...
{
    public partial class FrmLineup_KK : Form
    {
        ...
        private void FrmLineup_KK_Load(object sender, EventArgs e)
        {
            try
            {
                //fillCombo();
                ...
                Dt = new DataTable();
                Dt = objMB.ExeQueryStrRetDTBL("select TeamID,TeamName from Team_Master where TeamID in(select TeamA from KK_Matches where Matchid=" + Temp_matchid + " Union select TeamB from KK_Matches where Matchid=" + Temp_matchid + ")", 1);
                string tempstr = "select TeamID,TeamName from Team_Master where TeamID in(select TeamA from KK_Matches where Matchid=" + Temp_matchid + " Union select TeamB from KK_Matches where Matchid=" + Temp_matchid + ")";
        ...
        private void cbTeams_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                Dt = new DataTable();
                Dt = objMB.ExeQueryStrRetDTBL("select Pl.ID PlayerID, Pl.firstname +' '+ Pl.lastname PlayerName,JercyNo,IsLive from KK_Lineup L inner join Player_Master Pl on L.playerid=Pl.id where L.Matchid ='" + Temp_matchid + "' and L.ClubID='" + cbTeams.SelectedValue + "' order by Cast(JercyNo as int) asc", 1);
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (objMB.ExeQueryStrRetDTBL("Select * from KK_Lineup where matchid=" + Temp_matchid + " and ClubID=" + cbTeams.SelectedValue + "", 1).Rows.Count > 0)
                {
                    objMB.ExeQueryRetBooDL("Delete from KK_Lineup where matchid=" + Temp_matchid + " and ClubID=" + cbTeams.SelectedValue + "", 1);
                }
                foreach (DataGridViewRow dr in gvTeamDetails.Rows)
                {
                    ...
                    Query = "Insert Into KK_Lineup (MatchID,ClubID,PlayerId,IsSub,JercyNo,islive)values(" + Temp_matchid + "," + cbTeams.SelectedValue + "," + dr.Cells["PlayerId"].Value + "," + sub + "," + dr.Cells["JercyNo"].Value + ",'" + Live + "')";
