...
{
    public partial class FrmMatches_KK : Form
    {
        BusinessLy objMB = new BusinessLy();
        int ID = 0;
        DataTable DT;
        Boolean pageload = false;
        public FrmMatches_KK()
        {
            InitializeComponent();
            bindMatches(1);
        }

        public FrmMatches_KK(int MID)
        {
            InitializeComponent();
            bindMatches(2);
        }
        ...
        private void FrmMatches_KK_Load(object sender, EventArgs e)
        {
            bindData();
            ID = objMB.ExeQueryStrIntBL("Select Isnull(Max(MatchId),0)+1 from KK_Matches", 1);
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (btnSubmit.Text == "Submit")
                {
                    ...
                    Success = objMB.ExeQueryRetBooDL("Insert Into KK_Matches (MatchId,CompetitionId,VenueID,TeamA,TeamB,Date,MatchTypeID,ClubAColor,ClubBColor,StageType)values(" + ID + "," + cbCompetition.SelectedValue + "," + cbVenue.SelectedValue + "," + cbTeamA.SelectedValue + "," + cbTeamB.SelectedValue + ",'" + dtpMatchDate.Value + "'," + cbMatchType.SelectedValue + ",'" + mycommon.TeamAColor + "','" + mycommon.TeamBColor + "','" + cbstage.Text.Replace("'", "''") + "')", 1);
                ...
                else if (btnSubmit.Text == "Update")
                {
                    ...
                    Success = objMB.ExeQueryRetBooDL("Update KK_Matches set CompetitionId=" + cbCompetition.SelectedValue + ",VenueID=" + cbVenue.SelectedValue + ",TeamA=" + cbTeamA.SelectedValue + ",TeamB=" + cbTeamB.SelectedValue + ",Date='" + dtpMatchDate.Value + "',MatchTypeID=" + cbMatchType.SelectedValue + ",ClubAColor='" + mycommon.TeamAColor + "',ClubBColor='" + mycommon.TeamBColor + "',StageType='" + cbstage.Text.Replace("'", "''") + "' where MatchId=" + lblMatchID.Text.Trim() + "", 1);
        ...
        void bindData()
        {
            ...
            if (mycommon.SportID == 7)
            {
                ...
                cbstage.DataSource = objMB.ExeQueryStrRetDTBL("Select distinct StageType Stage from KK_Matches", 1);
        ...
        void bindMatches(int n)
        {
            if (n == 1)
            {
                cbName.DataSource = objMB.ExeQueryStrRetDTBL("Select MatchID,CAST(MatchID As Varchar)+' - '+dbo.fn_GetMatchName_KK(MatchId)Name from KK_Matches Order by date desc", 1);
            ...
            {
                cbName.DataSource = objMB.ExeQueryStrRetDTBL("Select MatchID,CAST(MatchID As Varchar)+' - '+dbo.fn_GetMatchName_KK(MatchId)Name from KK_Matches where MatchID="+mycommon.MatchId, 1);
                cbName.DisplayMember = "Name";
                cbName.ValueMember = "MatchID";
        ...
        private void cbName_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (cbName.SelectedIndex != -1)
                {
                    ...
                    DT = new DataTable();
                    DT = objMB.ExeQueryStrRetDTBL("Select * from KK_Matches where MatchID=" + cbName.SelectedValue + "", 1);
                    if (DT.Rows.Count > 0)
                    {
        ...
        {
            GlobalAssingvalues();
            FrmLineup_KK frmLine = new FrmLineup_KK();
            frmLine.Show();
        }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                if (objMB.ExeQueryRetBooDL("delete from BB_Matches where ID=" + lblMatchID.Text.Trim() + "", 1))
                {
