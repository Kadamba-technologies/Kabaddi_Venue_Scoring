...
{
    public partial class FrmLineup_VB : Form
    {
        ...
        int Temp_clubidA = CommonCls.TeamID1;
        int Temp_clubidB = CommonCls.TeamID2;
        SqlCommand SqlCmd;
        string Query = "";
        public FrmLineup_VB()
        {
        ...
        private void FrmLineup_VB_Load(object sender, EventArgs e)
        {
            try
            {
                //fillCombo();
                ...
                Dt = new DataTable();
                Dt = objMB.ExeQueryStrRetDTBL("select TeamID,TeamName from Team_Master where TeamID in(select TeamA from VB_Matches where Matchid=" + Temp_matchid + " Union select TeamB from VB_Matches where Matchid=" + Temp_matchid + ")", 1);
                string tempstr = "select TeamID,TeamName from Team_Master where TeamID in(select TeamA from VB_Matches where Matchid=" + Temp_matchid + " Union select TeamB from VB_Matches where Matchid=" + Temp_matchid + ")";
        ...
        public void GetPlayerBind()
        {
            try
            {
                ...
                lstPlayersList.DataSource = objMB.ExeQueryStrRetDTBL("select PlayerId ID,Cast(JercyNo As varchar)+' - '+dbo.fn_GetPlayerName(playerID) Name,JercyNo from CompetitionSquad Where TeamID=" + cbTeams.SelectedValue + " and CompId=" + CommonCls.CompId + " order by JercyNo asc", 1);
        ...
        private void cbTeams_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                Dt = new DataTable();
                Dt = objMB.ExeQueryStrRetDTBL("select Pl.ID PlayerID, Pl.firstname +' '+ Pl.lastname PlayerName,JercyNo,IsLive from VB_Lineup L inner join Player_Master Pl on L.playerid=Pl.id where L.Matchid ='" + Temp_matchid + "' and L.ClubID='" + cbTeams.SelectedValue + "' order by Cast(JercyNo as int) asc", 1);
        ...
        public int getJercyNo(int PlayerID)
        {
            ...
            {
                objMB = new BusinessLy();
                int JercyNo = objMB.ExeQueryStrIntBL("Select JercyNo from CompetitionSquad Where CompID=" + CommonCls.CompId + " and PlayerID=" + PlayerID + "", 1);
                return JercyNo;
            }
            catch
            {
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (objMB.ExeQueryStrRetDTBL("Select * from VB_Lineup where matchid=" + Temp_matchid + " and ClubID=" + cbTeams.SelectedValue + "", 1).Rows.Count > 0)
                {
                    objMB.ExeQueryRetBooDL("Delete from VB_Lineup where matchid=" + Temp_matchid + " and ClubID=" + cbTeams.SelectedValue + "", 1);
                }
                foreach (DataGridViewRow dr in gvTeamDetails.Rows)
                {
                    ...
                    Query = "Insert Into VB_Lineup (MatchID,ClubID,PlayerId,IsSub,JercyNo,islive)values(" + Temp_matchid + "," + cbTeams.SelectedValue + "," + dr.Cells["PlayerId"].Value + "," + sub + "," + dr.Cells["JercyNo"].Value + ",'" + Live + "')";
